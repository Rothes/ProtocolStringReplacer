    ...
    protected void process(@NotNull PacketEvent packetEvent) {
        ...
        if (packet.getIntegers().read(0) != 1) {
            if (ProtocolStringReplacer.getInstance().getServerMajorVersion() >= 13) {
                ...
                String replaced = AbstractServerPacketListener.getReplacedJson(packetEvent, user, listenType, wrappedChatComponent.getJson(), titleFilter);
                if (replaced != null) {
                    wrappedChatComponentStructureModifier.write(0, WrappedChatComponent.fromJson(replaced));
                }
            } else {
                StructureModifier<String> strings = packet.getStrings();
                String replaced = AbstractServerPacketListener.getReplacedText(packetEvent, user, listenType, strings.read(1), titleFilter);
                if (replaced != null)
